...
fun CalendarScreenContent(
    ...
    val allMusclePainEntriesWithMuscles by musclePainEntryMapViewModel.allMusclePainEntriesWithMuscles.observeAsState(
        listOf()
    )
    val dayEntryMap: Map<Date, CalendarEntry> =
        remember(allMusclePainEntriesWithMuscles, allWorkouts) {
    ...
    val monthsSortedEntries =
        remember(calendarState.monthState.currentMonth, allMusclePainEntriesWithMuscles) {
            ...
            for ((date, entry) in dayEntryMap) {
                ...
                if ((calendarsYearMonth.month.value - 1) == cal.get(Calendar.MONTH) && calendarsYearMonth.year == cal.get(
                        Calendar.YEAR
                    )
                ) {
